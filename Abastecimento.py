...
def salvar_ticketlog_supabase(df_ticket):
    ...
    # Mapeia colunas do relatório bruto para a tabela fato_abastecimento
    df_banco = pd.DataFrame({
        "codigo_transacao": df["Transação"].astype(str),
        "data_abastecimento": df["Data"],
        "hora_abastecimento": df["Hora"],
        "placa": df["PLACA"],
        "km_odometro": pd.to_numeric(
            df["Hodômetro Atual"], errors="coerce"
        ).fillna(0),
        ...
        "valor_total": pd.to_numeric(df["Valor Total"], errors="coerce").fillna(
            0
        ),
        "valor_unitario": pd.to_numeric(
            df["Valor/Litro"], errors="coerce"
        ).fillna(0),
    ...
    try:
        supabase.table("fato_abastecimento").upsert(
            registros, on_conflict="codigo_transacao"
        ).execute()
        print(
            f"✅ Supabase: {len(registros)} abastecimentos detalhados salvos com sucesso!"
        )
    except Exception as e:
...
def salvar_fechamento_supabase(df_relatorio):
    ...
    # Cria ID único: ex "2026_7_TBJ5C78"
    df["id"] = (
        df["ANO"].astype(str)
        + "_"
        + df["MES"].astype(str)
    ...
    try:
        ...
        print(
            f"✅ Supabase: {len(registros)} registros de fechamento salvos com sucesso!"
        )
    except Exception as e:
...
def extrair_gobrax(data_inicio_str, data_fim_str):
    ...
    def consultar_placa(placa, agrupamento):
        headers = {
            "Authorization": f"Bearer {TOKEN_GOBRAX}",
        ...
        for _ in range(3):
            try:
                r = requests.get(
                    URL_GOBRAX, headers=headers, params=params, timeout=30
                )
    ...
    with ThreadPoolExecutor(max_workers=15) as executor:
        futures = [
            executor.submit(consultar_placa, row.PLACA, row.agrupamento)
            for row in df_placas.itertuples()
        ]
        for f in tqdm(
            as_completed(futures), total=len(futures), desc="Gobrax (Placas)"
        ):
...
def extrair_ticketlog(data_inicio, data_fim):
    ...
    def consultar_dia_ticket(dia):
        ...
        for considerar in ["V", "T"]:
            payload = {
                ...
                "codigoTipoCartao": 4,
                "dataTransacaoInicial": inicio_dia,
                "dataTransacaoFinal": fim_dia,
            ...
            for _ in range(3):
                try:
                    r = requests.post(
                        URL_TICKET, json=payload, headers=headers, timeout=30
                    )
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(consultar_dia_ticket, d) for d in dias_consulta
        ]
        for f in tqdm(
            as_completed(futures), total=len(futures), desc="Ticketlog (Dias)"
        ):
            for t in f.result():
    ...
    df["quilometragem"] = pd.to_numeric(
        df["quilometragem"], errors="coerce"
    ).fillna(0)

    df = df.sort_values(by=["placa", "dataTransacao"])

    is_arla = (
        df["tipoCombustivel"].fillna("").str.upper().str.contains("ARLA")
    )
    ...
    df["Hodômetro Anterior"] = (
        df.groupby("placa")["Memoria_Hodometro"].shift(1).fillna(0)
    )

    df["KM Rodado"] = 0.0
    mask_valid = df["Hodometro_Valido"].notna()
    df.loc[mask_valid, "KM Rodado"] = (
        df["Hodometro_Valido"] - df["Hodômetro Anterior"]
    )

    df["KM Rodado"] = np.where(df["KM Rodado"] < 0, 0, df["KM Rodado"])
    df["KM Rodado"] = np.where(df["KM Rodado"] > 3000, 0, df["KM Rodado"])

    # Corte de memória (mantém apenas período solicitado)
...
def processar_relatorio(ano, mes):
    ...
    if not df_ticket.empty and "Litros" in df_ticket.columns:
        df_ticket["Litros"] = pd.to_numeric(
            df_ticket["Litros"], errors="coerce"
        ).fillna(0)
        df_ticket["Valor Total"] = pd.to_numeric(
            df_ticket["Valor Total"], errors="coerce"
        ).fillna(0)
    ...
    df_final = pd.merge(df_gobrax, df_ticket_geral, on="PLACA", how="outer").fillna(
        0
    )
    ...
    df_final["AGRUPAMENTO"] = df_final["AGRUPAMENTO"].replace(
        0, "NÃO INFORMADO"
    )
    ...
    print("\n☁️ Enviando dados para o Supabase (Power BI)...")
    salvar_ticketlog_supabase(df_ticket_completo)
    salvar_fechamento_supabase(df_relatorio)

    # SALVANDO O EXCEL E ENVIO DE E-MAIL
    agora = datetime.now().strftime("%H%M%S")
    nome_arquivo = f"Relatorio_Fechamento_{ano}_{mes:02d}_{agora}.xlsx"

    with pd.ExcelWriter(nome_arquivo, engine="openpyxl") as writer:
        df_ticket_completo.sort_values(by=["PLACA", "Data", "Hora"]).to_excel(
            writer, sheet_name="Aba 1 - Ticketlog Bruto", index=False
        )
        df_relatorio.to_excel(
            writer, sheet_name="Aba 2 - Relatorio Real", index=False
        )
...
def enviar_email(nome_arquivo, mes, ano):
    ...
    msg = EmailMessage()
    msg["Subject"] = (
        f"Relatório de Fechamento de Frota (Automatizado) - {mes:02d}/{ano}"
    )
    msg["From"] = EMAIL_USER
    msg["To"] = EMAIL_TO
    msg.set_content(
...
if __name__ == "__main__":
    ...
    print(
        f"🚀 A INICIAR AUTOMAÇÃO NO GITHUB ACTIONS | Mês de Ref: {mes_alvo:02d}/{ano_alvo}"
    )
