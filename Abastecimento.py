...
# =====================================================================
# 0. CREDENCIAIS DO SUPABASE (NOVO)
# =====================================================================
...
if SUPABASE_URL and SUPABASE_KEY:
    ...
    supabase = None
    print("⚠️ Supabase não configurado. Os dados serão gerados apenas no Excel.")

# =====================================================================
# 0.1 LISTA DE PLACAS BLOQUEADAS COMO 100% GNV (REGRA VIP)
# =====================================================================
PLACAS_100_GNV = [
...
# =====================================================================
# 2. FUNÇÕES DE EXTRAÇÃO DE DADOS (MANTIDAS 100% ORIGINAIS)
# =====================================================================
...
def processar_relatorio(ano, mes):
    ...
    # 🎯 CORREÇÃO DE DATA AQUI: Pega o último dia do mês, MAS trava no dia de hoje se o mês for atual.
    ultimo_dia_calculado = datetime(ano, mes, calendar.monthrange(ano, mes)[1], 23, 59, 59)
    ultimo_dia = min(ultimo_dia_calculado, datetime.now())
    ...
    # =================================================================
    # 🚀 INTEGRAÇÃO SUPABASE: Manda os dados exatos das abas para o BD
    # =================================================================
    if supabase and not df_ticket_completo.empty:
        print("\n[5/5] 📤 Gravando dados originais no Supabase...")
        
        # 1. Sobe Abastecimentos
        df_abast = df_ticket_completo.copy()
        df_abast = df_abast.rename(columns={
            "Transação": "codigo_transacao", "Data": "data_abastecimento", "Hora": "hora_abastecimento",
            "PLACA": "placa", "Hodômetro Anterior": "km_odometro", "KM Rodado": "km_rodado",
            "Posto": "posto", "Cidade": "cidade", "UF": "uf", "Combustível": "produto",
            "Litros": "litros", "Valor Total": "valor_total", "Valor/Litro": "valor_unitario",
            "Cartão": "cartao", "Status": "fonte_api"
        })
        dados_abast_dict = df_abast.where(pd.notnull(df_abast), None).to_dict(orient="records")
        try:
            supabase.table("fato_abastecimento").upsert(dados_abast_dict, on_conflict="codigo_transacao").execute()
            print(f"  ↳ ✅ {len(dados_abast_dict)} abastecimentos gravados!")
        except Exception as e:
            print(f"  ↳ ❌ Erro ao salvar abastecimentos: {e}")

        # 2. Sobe Fechamento
        df_fech_supa = df_relatorio.copy()
        df_fech_supa["id"] = df_fech_supa["ANO"].astype(str) + "_" + df_fech_supa["MES"].astype(str) + "_" + df_fech_supa["PLACA"]
        df_fech_supa = df_fech_supa.rename(columns={
            "ANO": "ano", "MES": "mes", "PLACA": "placa", "AGRUPAMENTO": "agrupamento",
            "KM Rodado Real": "km_rodado_real", "Consumo Real (L)": "consumo_real_l",
            "Média KM/L": "media_km_l", "Preço Médio Litro (R$)": "preco_medio_litro_rs",
            "Fonte do Dado": "fonte_do_dado", "DIESEL (L)": "diesel_l", "DIESEL (R$)": "diesel_rs",
            "GNV (L)": "gnv_l", "GNV (R$)": "gnv_rs", "ARLA 32 (R$)": "arla_32_rs"
        })
        dados_fech_dict = df_fech_supa.where(pd.notnull(df_fech_supa), None).to_dict(orient="records")
        try:
            supabase.table("fato_fechamento_frota").upsert(dados_fech_dict, on_conflict="id").execute()
            print(f"  ↳ ✅ {len(dados_fech_dict)} veículos de fechamento gravados!")
        except Exception as e:
            print(f"  ↳ ❌ Erro ao salvar fechamento: {e}")
...
def enviar_email(nome_arquivo, mes, ano):
    ...
    print(f"\n[E-MAIL] ✉️ A enviar e-mail com o relatório de {mes:02d}/{ano}...")
...
if __name__ == "__main__":
    ...
    # Se você quiser rodar a carga histórica de um mês específico, 
    # basta trocar manualmente aqui, exemplo: 
    # ano_alvo = 2026
    # mes_alvo = 7
